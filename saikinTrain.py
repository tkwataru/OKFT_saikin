...
CLASS_WEIGHT = 10.0     # saikin class weight for training
...
def parse_args(args=None):
    parser = argparse.ArgumentParser(
        description='Training saikinNet')
...
def load_model():
    model = SaikinNet()  # Set a neural network model
...
def _main():
    ...
    # Model
    model = load_model()    # Load a neural network model
    ...
    # evaluator = chainercv.extensions.SemanticSegmentationEvaluator(test_iter, model, label_names=('BG', 'Saikin'))
    ...
    trainer.extend(extensions.PlotReport(
        ...
        marker='',
        trigger=(1, 'iteration')))
    # Output a graph of Saikin class error
    trainer.extend(extensions.PlotReport(
