#!/usr/bin/env python
"""Semantic segmentation for Saikin. inference Saikin image list. Evaluate segmentation accuracy.
"""
...
def read_image(img_path):
    ...
    image = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)  # Read an image as gray scale
...
def get_pair(img_path, lab_path, augmentation=True):
    ...
    label = label.astype(np.int32) // 255  # Convert pixel values to label numbers
...
if __name__ == '__main__':
    csv_writer.writerow(['Number', 'BG IoU', 'Saikin IoU', 'Mean IoU', 'Pixel accuracy', 'BG accuracy', 'Saikin accuracy', 'Mean class accuracy', 'Name'])  # Write titles to csv file.
    ...
    for idx in range(len(img_list)):  # Index loop.
        ...
        image = image[np.newaxis, np.newaxis, :]    # Convert dimension to chainer form.
        ...
        result = chainercv.evaluations.eval_semantic_segmentation(seg_label[np.newaxis, :], label[:])   # Evaluate accuracy.
        print(result)
        ...
        layer2 = cv2.imread(img_path, cv2.IMREAD_UNCHANGED) // 4 * 3    # Read the original input image
